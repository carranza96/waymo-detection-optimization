...
flags.DEFINE_integer('save_checkpoints_secs', 3600, 'Save a checkpoint every n seconds')
flags.DEFINE_integer('gpu_device', None, 'Select GPU device')
flags.DEFINE_boolean('debug_tensorboard', False, 'If True, run Tensorboard Debug Hook')
...
def main(unused_argv):
    ...
    if FLAGS.checkpoint_dir:
        ...
        saving_listeners = [EvalCheckpointSaverListener(estimator, eval_specs[0].input_fn, 'validation')]
        if len(eval_specs) > 1:
            saving_listeners.append(EvalCheckpointSaverListener(estimator, eval_specs[1].input_fn, 'training'))
