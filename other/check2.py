...
FILENAME = "data/camera_data/training/training.record-00000-of-00798"
#FILENAME = "data/rgbRangeMedian/validation/validation.record-00001-of-00202"
#FILENAME = "data/rgbRange/training_eval.record"
...
for i, raw_example in enumerate(dataset):
        ...
        plt.title(camera_name,fontsize=16)
        # plt.grid(False)
        # plt.axis('off')
        for line in [0.2, 0.4, 0.7]:
            ax.axhline(y=image.get_shape().as_list()[0]*line, color='r', linestyle='--')
        fig2.savefig('fig4.svg', format='svg', dpi=500, bbox_inches='tight')
        plt.show()
